...
niitoGLM <- function(asmt, sublist, hemi, idx, df_cog, gmap_raw = TRUE, gmap_demeaned = TRUE) {
  ...
  for (i in 1:ncolumn){
    ...
    gradient <- gmap[i,]
    dataframe <- data.frame(y, gradient, sex=df_cog$sex, age=df_cog$age - mean(df_cog$age))
...
niitoGLM_poly3 <- function(asmt, sublist, hemi, idx, df_cog, gmap_raw = TRUE, gmap_demeaned = TRUE) {
  ...
  for (i in 1:ncolumn){
    if (i %% 500 == 0) {print(sprintf('->%d ', i))}
    gradient <- gmap[i,]
    dataframe <- data.frame(y, gradient, gradient2 = gradient^2, gradient3 = gradient^3, sex=df_cog$sex, age=df_cog$age - mean(df_cog$age))
...
gmap_lh <- niitogmap(df_cog$index, 'lh', raw = FALSE, demeaned = FALSE)
colnames(gmap_lh) <- df_cog$index
#gmap_rh <- niitogmap(df_cog$index, 'rh', raw = TRUE)
gmap_rh <- niitogmap(df_cog$index, 'rh', raw = FALSE, demeaned = FALSE)
...
for (i in 1:10) {
  ...
  dir <- sprintf("%s/boundary/GLM/gradient_age_age2_gm_cov/residual_cognitive/02212018_age_regressed/%s/%s", groupDir, cog_name, i)
  train_lh <- read.csv(paste0(dir, "/train_lh.csv"))[-1]
